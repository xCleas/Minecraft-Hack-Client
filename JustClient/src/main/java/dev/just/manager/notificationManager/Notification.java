...
public class Notification implements IMinecraft {
   ...
   float alpha = 1.0f;
   int times;
   private float width = 150.0f; // Varsayılan genişlik
   ...
   public Notification(NotificationType type, String name, String desc, int time) {
      ...
      this.desc = desc;
      this.times = time;
      // İlk genişliği hesapla
      try {
         if (mc != null && mc.textRenderer != null) {
            this.width = Math.max(mc.textRenderer.getWidth(name), mc.textRenderer.getWidth(desc)) + 50.0f;
         }
      } catch (Exception e) {
         this.width = 150.0f;
      }
      System.out.println("[NOTIFICATION] Created: " + name + " - " + desc + " (width=" + this.width + ")");
   }

   public float draw(DrawContext context) {
      try {
         // Font null check
         if (FontUtils.durman == null || FontUtils.durman[13] == null || FontUtils.durman[12] == null) {
            // Fallback: vanilla text renderer kullan
            this.width = mc.textRenderer.getWidth(this.name) + 50.0F;
            RenderUtil.drawRoundedRect(
               context.getMatrices(), this.x, this.y, this.width, 24.0F, 4.0F,
               ColorUtil.reAlphaInt(new Color(17, 15, 28, 255).getRGB(), (int)(170.0F * this.alpha))
            );
            context.drawText(mc.textRenderer, this.name, (int)(this.x + 5), (int)(this.y + 3), 0xFFFFFF, true);
            context.drawText(mc.textRenderer, this.desc, (int)(this.x + 5), (int)(this.y + 13), 0xD2D2D2, true);
            return 24.0F;
         }
         ...
         // Hata olursa basit render
         this.width = 100.0F;
         context.drawText(mc.textRenderer, this.name + " - " + this.desc, (int)this.x, (int)this.y, 0xFFFFFF, true);
      }
      return 24.0F;
   }
