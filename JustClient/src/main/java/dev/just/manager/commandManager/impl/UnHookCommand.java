...
public class UnHookCommand extends Command {
   ...
   public void execute(LiteralArgumentBuilder<CommandSource> builder) {
      builder.then(literal("path").then(arg("folder", StringArgumentType.greedyString()).suggests(this::suggestFolders).executes(context -> {
   ...
   private void savePath(String path) {
      try {
         ...
         try (FileWriter writer = new FileWriter(CUSTOM_PATH_FILE, false)) {
            ...
            ClientManager.message(Formatting.GREEN + I0O1l0I1.b("RG9zeWEgeW9sdSBrYXlkZWRpbGRpOiA=") + Formatting.WHITE + path);
         }
      } catch (IOException var7) {
         ClientManager.message(Formatting.RED + I0O1l0I1.b("WW9sIGtheWRlZGlsaXJrZW4gaGF0YSBvbHXFn3R1OiA=") + var7.getMessage());
      }
   }
}
