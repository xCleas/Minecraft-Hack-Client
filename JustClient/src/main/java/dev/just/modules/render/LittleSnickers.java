...
public class LittleSnickers extends Function {
   ...
   private final BooleanSetting followTarget = new BooleanSetting(I0O1l0I1.b("SGVkZWZlIFNhbGTEsXI="), false);
   private int ticksAway = 0;
   private int stuckTicks = 0;
   private LivingEntity currentTarget = null;

   private final EndTick tickListener = client -> onTick();
   ...
   private void onTick() {
      ...
      if (this.followTarget.get() && this.currentTarget != null) {
         double targetDist = this.currentTarget.getPos().distanceTo(this.fakeEntity.getPos());
         if (targetDist <= 12.0 && !this.currentTarget.isDead()) {
            moveTo = this.currentTarget.getPos();
            lookTarget = (Entity) this.currentTarget;
         } else {
   ...
   private void moveEntityTowardsTarget(Vec3d targetPos) {
      ...
      if (Math.abs(dy) >= 12.0 || distanceXZ > 20.0) {
         this.fakeEntity.refreshPositionAndAngles(targetPos.x, targetPos.y, targetPos.z, this.fakeEntity.getYaw(), this.fakeEntity.getPitch());
   ...
   public void onEvent(Event event) {
      if (event instanceof EventAttack attack && this.followTarget.get()) {
         if (attack.getTarget() instanceof LivingEntity living && living != mc.player) {
