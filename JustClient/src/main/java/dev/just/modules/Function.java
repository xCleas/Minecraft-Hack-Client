...
public abstract class Function implements IMinecraft {
   ...
   public final void setState(boolean enabled) {
      if (this.state != enabled) {
         ...
         // Zoom için autoSave çağırma (hold-to-use modül)
         if (!"Zoom".equals(this.name)) {
            autoSave();
         }
      }
   }
   ...
   private static final long SAVE_COOLDOWN = 1000; // 1 saniye cooldown

   public static void autoSave() {
      try {
         long now = System.currentTimeMillis();
         // Çok sık kaydetmeyi önle
         ...
         System.err.println("[AutoSave] Hata: " + e.getMessage());
         e.printStackTrace();
      }
   }
   ...
   public void toggle() {
      ...
      try {
         if (Manager.NOTIFICATION_MANAGER != null) {
            String statusText = this.state ? "Enabled" : "Disabled";
            System.out.println("[NOTIFICATION] Adding: " + this.name + " - " + statusText);
         ...
         System.err.println("[Notification] Hata: " + e.getMessage());
         e.printStackTrace();
      }
   ...
   private void playSound(boolean enable) {
      ...
      // ClientSounds her zaman aktif - state kontrolü yok
