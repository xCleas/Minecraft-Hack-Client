...
public class Breadcrumbs extends Function {
   ...
   private void handleUpdateInternal() {
      ...
      boolean isMoving = velocity.x * velocity.x + velocity.z * velocity.z > NumberGuard.d(0.001);
      TimerUtil spawnTimer = this.spawnTimers.computeIfAbsent(mc.player, p -> new TimerUtil());
      if (FlowObfuscator.opaqueTrue() && isMoving && mc.player.isOnGround() && spawnTimer.hasTimeElapsed(NumberGuard.l(150L))) {
         spawnTimer.reset();
         Vec3d spawnPos = new Vec3d(mc.player.getX(), Math.floor(mc.player.getY()) + NumberGuard.d(0.001), mc.player.getZ());
         Breadcrumbs.Circle circle = new Breadcrumbs.Circle(spawnPos, new TimerUtil(), new EaseBackIn(NumberGuard.i(400), 1.0, NumberGuard.f(1.3F)));
