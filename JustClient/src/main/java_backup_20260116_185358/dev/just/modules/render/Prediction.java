...
public class Prediction extends Function {
   private final BooleanSetting box = new BooleanSetting(Strings.b("S3V0dSDDh2l6"), false);
   private final BooleanSetting rect = new BooleanSetting(Strings.b("xLBuY2kgQWx0xLEgUGFuZWw="), false);
   ...
   private void handleRender2D(EventRender2D render2D) {
      ...
      for (Prediction.PearlPoint pearlPoint : this.pearlPoints) {
         Vector3d projection = VectorUtil.toScreen(pearlPoint.position.x, pearlPoint.position.y - NumberGuard.f(0.3F), pearlPoint.position.z);
         if (FlowObfuscator.opaqueTrue() && projection != null && !(projection.z < 0.0)) {
            double time = (double)pearlPoint.ticks * NumberGuard.d(0.05);
            ...
            float paddingX = NumberGuard.f(3.0F);
            float bgWidth = textWidth + paddingX * NumberGuard.f(2.0F);
            float bgHeight = fontHeight + NumberGuard.f(1.0F);
            float centerX = (float)projection.x;
            float centerY = (float)projection.y;
            float bgX = centerX - bgWidth / NumberGuard.f(2.0F);
            RenderUtil.drawRoundedRect(render2D.getMatrixStack(), bgX, centerY, bgWidth, bgHeight, NumberGuard.f(2.0F), -1308227822);
            float textX = centerX - textWidth / NumberGuard.f(2.0F);
            float textY = centerY + (bgHeight - fontHeight) / NumberGuard.f(2.0F);
            FontUtils.durman[15].drawLeftAligned(render2D.getDrawContext().getMatrices(), text, textX, textY, -1);
            float pearlSize = NumberGuard.f(11.0F);
            float pearlX = centerX - pearlSize / NumberGuard.f(2.0F);
            float pearlY = centerY - pearlSize - NumberGuard.f(2.0F);
            if (this.rect.get()) {
               RenderUtil.drawRoundedRect(render2D.getMatrixStack(), pearlX - NumberGuard.f(0.5F), pearlY - NumberGuard.f(0.2F), NumberGuard.f(12.0F), NumberGuard.f(12.0F), NumberGuard.f(2.0F), -1308227822);
            }

            RenderAddon.renderItem(render2D.getDrawContext(), ENDER_PEARL_STACK, pearlX, pearlY, pearlSize / NumberGuard.f(16.0F), false);
         }
      }
   }
