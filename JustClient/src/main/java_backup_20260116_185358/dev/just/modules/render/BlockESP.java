...
public class BlockESP extends Function {
   private final SliderSetting radius = new SliderSetting(Strings.b("WWFyxLHDp2Fw"), 20.0, 1.0, 30.0, 1.0);
   private final BooleanSetting chest = new BooleanSetting(Strings.b("U2FuZMSxa2xhcg=="), true);
   private final BooleanSetting furnace = new BooleanSetting(Strings.b("RsSxcsSxbmxhcg=="), true);
   private final BooleanSetting spawner = new BooleanSetting(Strings.b("U3Bhd25lcmxhcg=="), true);
   private final BooleanSetting brewingStand = new BooleanSetting(Strings.b("xLBrc2lyIFN0YW5kbGFyxLE="), true);
   private final BooleanSetting enderChest = new BooleanSetting(Strings.b("RW5kZXIgU2FuZMSxa2xhcsSx"), true);
   private final BooleanSetting detectorRail = new BooleanSetting(Strings.b("RGVkZWt0w7ZyIFJheWxhcg=="), true);
            ...
            for (int z = playerPos.getZ() - r; z <= playerPos.getZ() + r; z++) {
               ...
               if (!this.chest.get() || state.getBlock() != Blocks.CHEST && state.getBlock() != Blocks.TRAPPED_CHEST) {
                  if (!this.furnace.get()
                     ...
                     if (FlowObfuscator.opaqueTrue() && this.spawner.get() && state.getBlock() == Blocks.SPAWNER) {
                        RenderUtil.render3D.drawHoleOutline(box, new Color(255, 0, 255, 150).getRGB(), NumberGuard.f(2.0F));
                     } else if (this.brewingStand.get() && state.getBlock() == Blocks.BREWING_STAND) {
                        RenderUtil.render3D.drawHoleOutline(box, new Color(0, 191, 255, 150).getRGB(), NumberGuard.f(2.0F));
                     } else if (this.enderChest.get() && state.getBlock() == Blocks.ENDER_CHEST) {
                        RenderUtil.render3D.drawHoleOutline(box, new Color(75, 0, 130, 150).getRGB(), NumberGuard.f(2.0F));
                     } else if (this.detectorRail.get() && state.getBlock() == Blocks.DETECTOR_RAIL) {
                        RenderUtil.render3D.drawHoleOutline(box, new Color(255, 165, 0, 150).getRGB(), NumberGuard.f(2.0F));
                     } else {
                        String id = Registries.BLOCK.getId(state.getBlock()).toString();
                        Color c = this.customBlocks.get(id);
                        if (c != null) {
                           RenderUtil.render3D.drawHoleOutline(box, c.getRGB(), NumberGuard.f(2.0F));
                        }
                     }
                  } else {
                     RenderUtil.render3D.drawHoleOutline(box, new Color(128, 128, 128, 150).getRGB(), NumberGuard.f(2.0F));
                  }
               } else {
                  RenderUtil.render3D.drawHoleOutline(box, new Color(139, 69, 19, 150).getRGB(), NumberGuard.f(2.0F));
               }
            }
         }
      }
