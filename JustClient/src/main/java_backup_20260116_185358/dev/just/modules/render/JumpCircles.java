...
public class JumpCircles extends Function {
   private final ModeSetting circleType = new ModeSetting(Strings.b("SGFsa2EgVGlwaQ=="), "Tip-2", "Tip-1", "Tip-2", "Tip-3");
   private final SliderSetting rotateSpeed = new SliderSetting(Strings.b("RMO2bsO8xZ8gSMSxesSx"), 0.1F, 0.1F, 5.0, 0.1F);
   private final SliderSetting circleScale = new SliderSetting(Strings.b("Qm95dXQ="), 0.7F, 0.6F, 5.0, 0.1F);
   private final MultiSetting targets = new MultiSetting(Strings.b("R8O2csO8bsO8bQ=="), Arrays.asList(Strings.b("QXJrYWRhxZ9sYXI="), Strings.b("QmVuaQ==")), new String[]{Strings.b("T3l1bmN1bGFy"), Strings.b("QXJrYWRhxZ9sYXI="), Strings.b("QmVuaQ==")});
   ...
   private void handleUpdateInternal() {
      FlowObfuscator.fakeHandler();
      this.circles.removeIf(c -> c.timer.getTime() > NumberGuard.l(8000L));

      for (PlayerEntity player : Manager.SYNC_MANAGER.getPlayers()) {
         if (FlowObfuscator.opaqueTrue() && player != null && !player.isRemoved()) {
            ...
            boolean showPlayers = this.targets.get(Strings.b("T3l1bmN1bGFy"));
            boolean showFriends = this.targets.get(Strings.b("QXJrYWRhxZ9sYXI="));
            boolean showMe = this.targets.get(Strings.b("QmVuaQ=="));
            ...
            if (!shouldTrack) {
               ...
               if (previouslyOnGround && !currentlyOnGround) {
                  JumpCircles.Circle circle = new JumpCircles.Circle(
                     new Vec3d(player.getX(), Math.floor(player.getY()) + NumberGuard.d(0.001), player.getZ()),
                     new TimerUtil(),
                     new EaseBackIn(NumberGuard.i(400), 1.0, NumberGuard.f(1.3F))
                  );
