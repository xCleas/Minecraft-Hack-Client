...
public class AutoPotion extends Function {
   private final BooleanSetting autoOff = new BooleanSetting(Strings.b("T3RvbWF0aWsgS2FwYXRtYQ=="), false);
   public MultiSetting potions = new MultiSetting(Strings.b("xLBrc2lybGVy"), Arrays.asList(Strings.b("R8O8w6c="), Strings.b("SMSxeg=="), Strings.b("QXRlxZ8gRGlyZW5jaQ==")), new String[]{Strings.b("R8O8w6c="), Strings.b("SMSxeg=="), Strings.b("QXRlxZ8gRGlyZW5jaQ==")});
   ...
   private int isEatingFoodFlag() {
      ...
      return NumberGuard.bool(LogicSplit.all(
         ...
         LogicSplit.not(mc.player.getActiveItem().isOf(Items.SHIELD)),
         LogicSplit.not(mc.player.getActiveItem().isOf(Items.BOW)),
         LogicSplit.not(mc.player.getActiveItem().isOf(Items.TRIDENT))
      ));
   }

   private boolean isEatingFoodInternal() {
      return NumberGuard.unbool(isEatingFoodFlag());
   }
   ...
   private int hasEffectFlag(RegistryEntry<StatusEffect> effect) {
      FlowObfuscator.fakeHandler();
      return NumberGuard.bool(mc.player.hasStatusEffect(effect));
   }

   private boolean hasEffectInternal(RegistryEntry<StatusEffect> effect) {
      return NumberGuard.unbool(hasEffectFlag(effect));
   }
   ...
   private int canBuffTypeFlag(AutoPotion.PotionType type) {
      ...
      return NumberGuard.bool(LogicSplit.and(type.isEnabled(this), findPotionSlotInternal(type) != NumberGuard.i(-1)));
   }

   private boolean canBuffTypeInternal(AutoPotion.PotionType type) {
      return NumberGuard.unbool(canBuffTypeFlag(type));
   }

   private int canBuffFlag() {
      FlowObfuscator.fakeHandler();
      if (FlowObfuscator.opaqueFalse()) {
         return NumberGuard.bool(entropy > FAKE_DELAY);
      }
      return NumberGuard.bool(LogicSplit.all(
         LogicSplit.not(isEatingFoodInternal()),
         LogicSplit.or(canBuffTypeInternal(PotionType.STRENGTH), canBuffTypeInternal(PotionType.SPEED), canBuffTypeInternal(PotionType.FIRE_RESISTANCE)),
         mc.player.isOnGround(),
         this.timer.hasTimeElapsed(NumberGuard.l(500))
      ));
   }

   private boolean canBuffInternal() {
      return NumberGuard.unbool(canBuffFlag());
   }
   ...
   private boolean isActiveInternal() {
      return NumberGuard.unbool(isActiveFlag());
   }
   ...
   private void handleMotionInternal(EventMotion eventAfterRotate) {
      ...
      if (shouldThrowInternal()) {
         ...
         eventAfterRotate.setPitch(NumberGuard.f(90.0F));
         this.spoofed = true;
   ...
   private void handleUpdateInternal() {
      ...
      while (true) {
         switch (_s) {
            case 0:
               FlowObfuscator.fakeHandler();
               if (LogicSplit.and(this.isActivePotion, LogicSplit.not(shouldThrowInternal()))) {
            ...
            case 1:
               if (LogicSplit.and(shouldThrowInternal(), this.spoofed)) {
            ...
            case 2:
               if (FlowObfuscator.opaqueFalse()) {
                  FlowObfuscator.fakeBranch(spoofed, entropy);
               }
               _s = 4;
               break;
   ...
   private boolean shouldThrowInternal() {
      return NumberGuard.unbool(shouldThrowFlag());
   }

   private void throwPotionInternal(AutoPotion.PotionType type) {
      FlowObfuscator.fakeHandler();
      if (LogicSplit.and(type.isEnabled(this), LogicSplit.not(hasEffectInternal(type.effect)))) {
         int slot = findPotionSlotInternal(type);
         if (slot != NumberGuard.i(-1)) {
   ...
   private void sendSequencedPacketInternal(SequencedPacketCreator packetCreator) {
      FlowObfuscator.fakeHandler();
      if (LogicSplit.or(mc.player.networkHandler == null, mc.world == null)) return;
   ...
   private static enum PotionType {
      STRENGTH(5, StatusEffects.STRENGTH, Strings.b("R8O8w6c=")),
      SPEED(1, StatusEffects.SPEED, Strings.b("SMSxeg==")),
      FIRE_RESISTANCE(12, StatusEffects.FIRE_RESISTANCE, Strings.b("QXRlxZ8gRGlyZW5jaQ=="));
