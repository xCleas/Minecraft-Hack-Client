...
public class SelfTrap extends Function {
   ...
   public void onEnable() {
      ...
      if (LogicSplit.and(mc.player != null, mc.world != null)) {
         this.targets.clear();
         ...
         this.index = NumberGuard.i(0);
         this.active = true;
         this.tickDelay = NumberGuard.i(0);
      } else {
         this.toggle();
      }
   }
   ...
   public void onDisable() {
      FlowObfuscator.fakeHandler();
      if (LogicSplit.and(mc.player != null, this.originalSlot != NumberGuard.i(-1))) {
   ...
   private void handleUpdateEventInternal() {
      ...
      while (true) {
         switch (_s) {
            case 0:
               FlowObfuscator.fakeHandler();
               if (this.tickDelay > NumberGuard.i(0)) {
                  this.tickDelay--;
                  _s = 5;
                  break;
               }
            ...
            case 2:
               ...
               if (LogicSplit.or(!mc.world.getBlockState(target).isAir(), this.isEntityBlocking(target))) {
                  this.index++;
                  _s = 5;
                  break;
               }
   ...
   private void processBlockPlacementInternal() {
      ...
      if (placeSide != null) {
         ...
         Vec3d hitVec = Vec3d.ofCenter(neighbor).add(Vec3d.of(placeSide.getVector()).multiply(NumberGuard.d(0.5)));
         int blockSlot = this.findAnyBlockInHotbar();

         if (blockSlot == NumberGuard.i(-1)) {
            this.finish();
            return;
         }
         ...
         this.tickDelay = NumberGuard.i(1) + this.rnd.nextInt(NumberGuard.i(3));
         this.index++;
         return;
      }
