...
public class NoDelay extends Function {
   ...
   private void handleItemDelayInternal() {
      ...
      while (true) {
         switch (_s) {
            ...
            case 1:
               if (FlowObfuscator.opaqueFalse()) {
                  ((MinecraftClientAccessor)mc).setUseCooldown(NumberGuard.i(FAKE_COOLDOWN));
                  _s = 4;
                  break;
               }
               _s = 2;
            ...
            case 2:
               if (checkItemInternal(item)) {
                  if (FlowObfuscator.opaqueTrue()) {
                     ((MinecraftClientAccessor)mc).setUseCooldown(NumberGuard.i(0));
                  }
               }
               _s = 4;
               break;
            ...
            case 3:
               if (FlowObfuscator.opaqueFalse()) {
                  FlowObfuscator.fakeBranch(item, entropy);
               }
               _s = 4;
               break;
   ...
   private boolean checkItemInternal(Item item) {
      return NumberGuard.unbool(checkItemFlag(item));
   }

   private boolean checkBlockItemInternal(Item item) {
      FlowObfuscator.fakeHandler();
      return LogicSplit.all(item instanceof BlockItem, this.place.get());
   }

   private boolean checkCrystalInternal(Item item) {
      FlowObfuscator.fakeHandler();
      return LogicSplit.all(item == Items.END_CRYSTAL, this.crystal.get());
   }

   private boolean checkXpBottleInternal(Item item) {
      FlowObfuscator.fakeHandler();
      return LogicSplit.all(item == Items.EXPERIENCE_BOTTLE, this.xp.get());
   }
