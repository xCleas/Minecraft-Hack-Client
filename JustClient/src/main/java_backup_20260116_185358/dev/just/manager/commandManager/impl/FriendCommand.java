...
public class FriendCommand extends Command {
   ...
   public void execute(LiteralArgumentBuilder<CommandSource> builder) {
      builder.then(literal("add").then(arg("player", PlayerArgumentType.create()).executes(context -> {
         ...
         if (Manager.FRIEND_MANAGER.isFriend(name)) {
            ClientManager.message(Formatting.RED + Strings.b("SGF0YSEg") + Formatting.GOLD + name + Formatting.WHITE + Strings.b("IHphdGVuIGFya2FkYcWfIGxpc3Rlc2luZGUu"));
            return 1;
         } else {
            Manager.FRIEND_MANAGER.addFriend(name);
            ClientManager.message(Formatting.GREEN + Strings.b("QmHFn2FyxLFsxLEhIA==") + Formatting.GOLD + name + Formatting.WHITE + Strings.b("IGFya2FkYcWfIGxpc3Rlc2luZSBla2xlbmRpLg=="));
            return 1;
         }
      })));
      builder.then(literal("remove").then(arg("player", FriendArgumentType.create()).executes(context -> {
         ...
         ClientManager.message(Formatting.GREEN + Strings.b("QmHFn2FyxLFsxLEhIA==") + Formatting.GOLD + name + Formatting.WHITE + Strings.b("IGFya2FkYcWfIGxpc3Rlc2luZGVuIHNpbGluZGku"));
         return 1;
      })));
      ...
      builder.then(literal("list").executes(context -> {
         if (Manager.FRIEND_MANAGER.getFriends().isEmpty()) {
            ClientManager.message(Formatting.GRAY + Strings.b("QXJrYWRhxZ8gbGlzdGVzaSBib8WfLg=="));
         } else {
            String friendsList = Manager.FRIEND_MANAGER.getFriends().stream().map(f -> f.getName()).reduce((a, b) -> a + ", " + b).orElse("");
            ClientManager.message(Formatting.GREEN + Strings.b("QXJrYWRhxZ9sYXI6IA==") + Formatting.WHITE + friendsList);
         }
   ...
   private void clearFriendList() {
      ...
      if (friendManager.getFriends().isEmpty()) {
         ClientManager.message(Formatting.RED + Strings.b("SGF0YSEg") + Formatting.WHITE + Strings.b("QXJrYWRhxZ8gbGlzdGVzaSB6YXRlbiBib8WfLg=="));
      } else {
         friendManager.clearFriends();
         ClientManager.message(Formatting.GREEN + Strings.b("QmHFn2FyxLFsxLEhIA==") + Formatting.WHITE + Strings.b("QXJrYWRhxZ8gbGlzdGVzaSB0ZW1pemxlbmRpLg=="));
      }
   }
}
