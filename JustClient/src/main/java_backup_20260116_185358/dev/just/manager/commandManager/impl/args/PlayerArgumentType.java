...
public class PlayerArgumentType implements ArgumentType<String>, IMinecraft {
   ...
   public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
      List<String> onlinePlayers = mc.player.networkHandler.getPlayerList().stream().map(entry -> entry.getProfile().getName()).toList();
      return CommandSource.suggestMatching(onlinePlayers, builder);
   }

   public Collection<String> getExamples() {
      return List.of("Steve", "Alex", "Notch", "JustPlayer");
   }
}
