...
public class ConfigCommand extends Command {
   ...
   public void execute(LiteralArgumentBuilder<CommandSource> root) {
      root.then(literal("save").then(arg("name", StringArgumentType.word()).suggests((ctx, builder) -> {
         ...
         ClientManager.message(name + Strings.b("IGlzaW1saSB5YXDEsWxhbmTEsXJtYSBrYXlkZWRpbGRp"));
         return 1;
      })));
      ...
      root.then(literal("remove").then(arg("name", StringArgumentType.word()).suggests((ctx, builder) -> {
         ...
         ClientManager.message(name + Strings.b("IHlhcMSxbGFuZMSxcm1hc8SxIHNpbGluZGk="));
         return 1;
      })));
      root.then(literal("list").executes(ctx -> {
         List<String> configs = Manager.CONFIG_MANAGER.getAllConfigurations();
         if (configs.isEmpty()) {
            ClientManager.message(Strings.b("WWFwxLFsYW5kxLFybWEgbGlzdGVzaSBib8Wf"));
         } else {
            ClientManager.message(Strings.b("WWFwxLFsYW5kxLFybWEgbGlzdGVzaTo="));
      ...
      root.then(literal("reset").executes(ctx -> {
         ...
         ClientManager.message(Strings.b("QXlhcmxhciBzxLFmxLFybGFuZMSx"));
         return 1;
      }));
      root.then(literal("dir").executes(ctx -> {
         try {
            ...
            ClientManager.message(Strings.b("S2xhc8O2ciBhw6fEsWzEsXJrZW4gaGF0YSBvbHXFn3R1OiA=") + var2.getMessage());
         }
      ...
      root.then(literal("clear").executes(ctx -> {
         List<String> configs = Manager.CONFIG_MANAGER.getAllConfigurations();
         if (configs.isEmpty()) {
            ClientManager.message(Strings.b("WWFwxLFsYW5kxLFybWEgbGlzdGVzaSB6YXRlbiBib8Wf"));
            return 1;
         } else {
            configs.forEach(Manager.CONFIG_MANAGER::deleteConfig);
            ClientManager.message(Strings.b("VMO8bSB5YXDEsWxhbmTEsXJtYWxhciBzaWxpbmRp"));
            return 1;
         }
      }));
   }
