...
public class GpsCommand extends Command {
   ...
   public void execute(LiteralArgumentBuilder<CommandSource> builder) {
      ...
      builder.then(literal("off").executes(context -> {
         enabled = false;
         GPS_POSITION = null;
         ClientManager.message(Formatting.GRAY + Strings.b("R1BTIGthcGF0xLFsZMSxIQ=="));
         return 1;
      }));
   }
   ...
   private static void setGps(int x, int z) {
      ...
      enabled = true;
      ClientManager.message(Formatting.GRAY + Strings.b("R1BTIGF5YXJsYW5kxLE6IFg6IA==") + x + ", Y: " + (int)mc.player.getY() + ", Z: " + z);
   }
   ...
   public static void render(MatrixStack matrixStack) {
      if (enabled && GPS_POSITION != null && mc.player != null) {
         ...
         if (distance < 1) {
            FontUtils.durman[15].centeredDraw(matrixStack, Strings.b("SGVkZWZlIHVsYcWfdMSxbsSxeiE="), screenWidth, screenHeight - 105.0F, -1);
         } else {
            matrixStack.push();
