   ...
   public void execute(LiteralArgumentBuilder<CommandSource> root) {
      root.then(literal("add").then(arg("name", StringArgumentType.word()).suggests(this::suggestOnlinePlayers).executes(ctx -> {
         ...
         if (Manager.STAFF_MANAGER.getStaffNames().contains(name)) {
            ClientManager.message(Formatting.RED + Strings.b("QnUgb3l1bmN1IHphdGVuIFlldGtpbGkgTGlzdGVzaW5kZSE="));
         } else {
            Manager.STAFF_MANAGER.addStaff(name);
            ClientManager.message(Formatting.GREEN + Strings.b("S3VsbGFuxLFjxLEgYWTEsSA=") + Formatting.WHITE + name + Formatting.GREEN + Strings.b("IHlldGtpbGkgbGlzdGVzaW5lIGVrbGVuZGku"));
         }
                     ...
                     ctx -> {
                        ...
                        if (Manager.STAFF_MANAGER.getStaffNames().contains(name)) {
                           ...
                           ClientManager.message(
                              Formatting.GREEN + Strings.b("S3VsbGFuxLFjxLEgYWTEsSA=") + Formatting.WHITE + name + Formatting.GREEN + Strings.b("IHlldGtpbGkgbGlzdGVzaW5kZW4gc2lsaW5kaS4=")
                           );
                        } else {
                           ClientManager.message(Formatting.RED + Strings.b("QnUgb3l1bmN1IHlldGtpbGkgbGlzdGVzaW5kZSBidWx1bmFtYWTEsSE="));
                        }
      ...
      root.then(literal("clear").executes(ctx -> {
         if (Manager.STAFF_MANAGER.getStaffNames().isEmpty()) {
            ClientManager.message(Formatting.RED + Strings.b("WWV0a2lsaSBsaXN0ZXNpIHphdGVuIGJvxZ8h"));
         } else {
            Manager.STAFF_MANAGER.clearStaffs();
            ClientManager.message(Formatting.GREEN + Strings.b("WWV0a2lsaSBsaXN0ZXNpIHRlbWl6bGVuZGku"));
         }
      ...
      root.then(literal("list").executes(ctx -> {
         if (Manager.STAFF_MANAGER.getStaffNames().isEmpty()) {
            ClientManager.message(Formatting.GRAY + Strings.b("WWV0a2lsaSBsaXN0ZXNpIGJvxZ8u"));
         } else {
            ClientManager.message(Formatting.GRAY + Strings.b("S2F5xLF0bMSxIFlldGtpbGlsZXI6"));
      ...
      root.then(literal("reload").executes(ctx -> {
         ...
         ClientManager.message(Formatting.GREEN + Strings.b("WWV0a2lsaSBsaXN0ZXNpIGRvc3lhZGFuIHllbmlkZW4gecO8a2xlbmRpLg=="));
         return 1;
      }));
   }
